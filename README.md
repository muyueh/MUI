...
- `.github/workflows/deploy.yml` 會在 `work`、`main`、或 `codex/*` 分支推送，以及手動觸發時自動：
  1. 設定 Pages 環境
  2. 執行 `npm ci && npm run build`
  3. 上傳 `dist/` 為 Pages artifact 並發布到 GitHub Pages
- 若分支不在上述清單或未手動觸發 workflow，GitHub Pages 不會產出 artifact，因此頁面顯示為空白是正常的；請推送到支援分支或於 Actions 介面執行「Deploy to GitHub Pages」。
